...
- **M2.5 COMPLETE** — hot-seat 1v1 skirmish: VERSUS menu, 4 playable ships (both factions, cloak included), hidden-order WEGO flow with a console-handoff privacy screen, POV-aware rendering (a cloaked ship ghosts only on its own captain's console; replays are public view), hotseat transport behind the same interface M6's WebRTC peer will use, outcome/rematch screen. Same-class duels get accent tinting.
- **Next: M3** — crew & officers, hailing/diplomacy system, faction reputation, encounters that flow between combat and talk.
